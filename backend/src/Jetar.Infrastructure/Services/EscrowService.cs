...
public class EscrowService : IEscrowService
{
    ...
    public async Task<Transaction> InitiateAsync(Guid listingId, Guid buyerId, CancellationToken ct = default)
    {
        ...
        var buyer = await _db.Users.FirstOrDefaultAsync(u => u.Id == buyerId, ct)
        ...
        var existing = await _db.Transactions.FirstOrDefaultAsync(
    ...
    public async Task<int> ReleaseExpiredAsync(CancellationToken ct = default)
    {
        ...
        var due = await _db.Transactions
            .Include(t => t.Listing)
            .Include(t => t.Seller)
            .Where(t => (t.Status == TransactionStatus.CredentialsSent || t.Status == TransactionStatus.BuyerVerifying)
    ...
    public async Task<Dispute> OpenDisputeAsync(Guid transactionId, Guid userId, OpenDisputeRequest request, CancellationToken ct = default)
    {
        ...
        var already = await _db.Disputes.FirstOrDefaultAsync(d => d.TransactionId == transactionId, ct);
    ...
    public async Task<int> CancelStaleUnpaidAsync(CancellationToken ct = default)
    {
        ...
        var stale = await _db.Transactions
            .Include(t => t.Listing)
            .Where(t => (t.Status == TransactionStatus.Initiated || t.Status == TransactionStatus.AwaitingPayment)
                        && t.CreatedAt <= cutoff)
            .ToListAsync(ct);
    ...
    private async Task CompleteAsync(Transaction tx, string systemMessage, CancellationToken ct)
    {
        ...
        var seller = tx.Seller ?? await _db.Users.FirstOrDefaultAsync(u => u.Id == tx.SellerId, ct);
        if (seller != null) seller.TotalSales++;

        var buyer = tx.Buyer ?? await _db.Users.FirstOrDefaultAsync(u => u.Id == tx.BuyerId, ct);
    ...
    private async Task ApplyRefundAsync(Transaction tx, string reason, CancellationToken ct)
    {
        ...
        var payments = await _db.Payments
            .Where(p => p.TransactionId == tx.Id && p.Status == PaymentStatus.Paid)
            .ToListAsync(ct);
