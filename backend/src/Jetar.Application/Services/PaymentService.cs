...
public class PaymentService : IPaymentService
{
    ...
    public async Task<PaymentDto> CreateAsync(Guid transactionId, Guid userId, PaymentMethod method, CancellationToken ct = default)
    {
        ...
        var stale = await _uow.Payments.ListAsync(new PendingPaymentsForTransactionSpec(transactionId), ct);
        ...
        PaymentMethod method, string rawPayload, IDictionary<string, string> headers, CancellationToken ct = default)
    {
        ...
        else await _uow.SaveChangesAsync(ct);
    ...
    {
        var payment = await _uow.Payments.FirstOrDefaultAsync(p => p.ProviderPaymentId == providerPaymentId, ct)
    ...
    {
        var payments = await _uow.Payments.ListAsync(new PaidPaymentsForTransactionSpec(transactionId), ct);
